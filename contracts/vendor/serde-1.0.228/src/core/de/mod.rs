...
impl<'a> fmt::Display for Unexpected<'a> {
    fn fmt(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
        ...
        match *self {
            ...
            Float(_) => formatter.write_str("floating point value"),
...
pub trait Visitor<'de>: Sized {
    ...
    {
        Err(Error::invalid_type(
            Unexpected::Other("floating point value"),
            &self,
        ))
    }
...
// Floating point formatting is intentionally omitted to avoid bringing float
// instructions into the Wasm artifact.
